    ...
    public class InstituicaoController : BaseController
    {
        ...
        public async Task<IActionResult> SelecionarInstituicao(string? search, int? page = 1, bool? ativo = true)
        {
            ...
            var paged = new PagedViewModel<InstituicaoViewModel>
            {
                ...
                Search = search,
                Page = page.Value,
                PageSize = (int)EnumQuantidadeDeItensPorPagina.Alguns,
        ...
        public async Task<IActionResult> Upsert(Guid? id)
        {
            ...
            ViewBag.EstadoInicialIntegral = instituicaoViewModel.Integral.GetValueOrDefault(false);
        ...
        public async Task<IActionResult> Upsert(InstituicaoViewModel instituicaoViewModel)
        {
            ...
            bool? estadoDoToggleEnviadoAtivo = instituicaoViewModel.Ativo;
            bool? estadoDoToggleEnviadoIntegral = instituicaoViewModel.Integral;
            ...
            else
            {
                ...
                bool houveAlteracao =
                    instituicaoExistente.Nome != instituicaoViewModel.Nome ||
                    instituicaoExistente.CodigoCie != instituicaoViewModel.CodigoCie ||
                    instituicaoExistente.Cnpj != instituicaoViewModel.Cnpj ||
                    instituicaoExistente.Integral != instituicaoViewModel.Integral ||
                    instituicaoExistente.Ativo != instituicaoViewModel.Ativo;
                ...
                if (!houveAlteracao)
                {
                    return Json(new
                    {
                        success = false,
                        type = "warning",
                        errors = new[] { "Nenhuma alteração localizada." }
                    });
                }
