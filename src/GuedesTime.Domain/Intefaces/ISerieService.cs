...
namespace GuedesTime.Domain.Intefaces
{
	public interface ISerieService : IDisposable
	{
        Task<PagedResult<Serie>> GetPagedByInstituicaoAsync(
                    ...
                    int pageSize,
                    bool ativo,
                    Expression<Func<Serie, bool>>? filtroAdicional,
                    Func<IQueryable<Serie>, IOrderedQueryable<Serie>>? ordenacao,
                    IQueryable<Serie>? sourceQuery,
        ...
        Task<Serie> ObterPorIdComDisciplinasAsync(Guid id);
        Task SincronizarDisciplinasAsync(Guid serieId, IEnumerable<Guid> novasDisciplinaIds);
        Task<PagedResult<Serie>> ObterSeriesPaginadoComDisciplinas(Guid instituicaoId, string? search, int page, int pageSize, bool ativo, EnumTipoEnsino? tipoEnsino = null);
    }
}